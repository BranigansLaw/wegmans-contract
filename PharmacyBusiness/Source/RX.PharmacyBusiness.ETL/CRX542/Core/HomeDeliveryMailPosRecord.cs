namespace RX.PharmacyBusiness.ETL.CRX542.Core
{
    ...
    public class HomeDeliveryMailPosRecord : PosRecord, IRecord
    {
        public void SetValuesFromInputFields(IEnumerable<string> fields)
        {
            ...
            string tempOrder_Num = string.Empty;
            int tempTransaction_Num = 0;
            if (string.IsNullOrEmpty(fields.ElementAt(4)))
                constraintViolations.AppendLine("Order_Num cannot be blank.");
            else if (fields.ElementAt(4).Length > 38)
                constraintViolations.AppendLine("Order_Num is longer than the max 38 characters.");
            else
            { 
                tempOrder_Num = fields.ElementAt(4);

                // All Order Numbers begin with one letter we can remove to get the number.
                if (!int.TryParse(tempOrder_Num.Substring(1, tempOrder_Num.Length - 1), out tempTransaction_Num))
