...
REM Two date parameters, even if same dates, are required for the third parameter FilenameModification to be picked up.
@echo off &SETLOCAL
SET START_DATE=%~1
SET END_DATE=%~2
SET FILE_NAME_MODIFICATION=%~3
...
IF "%3"=="" (
    SET FILE_NAME_MODIFICATION=_Modified
)

@ECHO Date range is from %START_DATE% to %END_DATE% with FILE_NAME_MODIFICATION of %FILE_NAME_MODIFICATION%.
...
:loop
    ...
    %BATCH_ROOT%\PharmacyBusiness\bin\Wegmans.InterfaceEngine.exe -a RX.PharmacyBusiness.ETL.dll -i DownloadForCDC -RunDate %runDate% -FilenameModification %FILE_NAME_MODIFICATION%
    IF %ERRORLEVEL% NEQ 0 (
