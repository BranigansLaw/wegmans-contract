...
:loop
    ...
    %BATCH_ROOT%\PharmacyBusiness\bin\Wegmans.InterfaceEngine.exe -a RX.PharmacyBusiness.ETL.dll -i NYNonResidentZeroReports -RunDate %runDate%
    IF %ERRORLEVEL% NEQ 0 (
        @ECHO "Error with Run Date %runDate% !!!""
        EXIT /B %ERRORLEVEL%
    )
    IF %endJulian% gtr %startJulian% (
        SET /a startJulian=startJulian+7
        goto loop
    )
EXIT /B 0
