namespace RX.PharmacyBusiness.ETL
{
    ...
    public class SqlServerHelper
    {
           ...
           string connectionName)
        {
            ...
            SqlParameter pRunDate = new SqlParameter("@RunDate", SqlDbType.Date);
            pRunDate.Value = runDate;
            SqlConnection sqlsCon = null;

            try
            {
                sqlsCon = new SqlConnection(ConfigurationManager.ConnectionStrings[connectionName].ToString());
                sqlsCon.Open();

                SqlCommand cmd = new SqlCommand(selectSql, sqlsCon);
            ...
            string connectionName)
        {
            ...
            try
            {
                ...
                sqlsCon.Open();
                queryStartTime = DateTime.Now;
                returnList = sqlsCon.Query<T>(selectSql, new { RunDate = runDate }, commandTimeout: 300).ToList();
                queryStopTime = DateTime.Now;
                sqlsCon.Close();
            }
            finally
        ...
        public void ExecuteDeleteFromTable(
            string sqlFile,
            string connectionName)
        {
            List<EmptyRecord> deleteOnce = new List<EmptyRecord>();
            deleteOnce.Add(new EmptyRecord() { emptyString = "DeleteOnce" });
            ExecuteNonQueryFromList<EmptyRecord>(
                sqlFile,
                deleteOnce,
                connectionName);
        }

        public bool ExecuteNonQueryFromList<T>(
            string sqlFile,
            List<T> dataList,
            string connectionName)
        {
            ...
            Log.LogInfo("Executing non-query file [{0}].", sqlFile);
            DateTime queryStartTime;
            DateTime queryStopTime;
            string nonQuerySql = string.Empty;
            using (var fileReader = new StreamReader(this.fileManager.OpenRead(sqlFile)))
            {
                nonQuerySql = fileReader.ReadToEnd();
            }

            using (SqlConnection sqlsCon = new SqlConnection(ConfigurationManager.ConnectionStrings[connectionName].ToString()))
            using (SqlCommand command = new SqlCommand())
            {
                command.Connection = sqlsCon;
                command.CommandType = CommandType.Text;
                command.CommandText = nonQuerySql;
                ...
                try
                {
                    ...
                    {
                        foreach (PropertyInfo prop in dataRecord.GetType().GetProperties())
                        {
                            command.Parameters.AddWithValue("@" + prop.Name, prop.GetValue(dataRecord, null).ToString());
                        }
            ...
            TimeSpan tsDifferenceForQuery = queryStopTime - queryStartTime;
            double tsSecondsForQuery = Math.Round(tsDifferenceForQuery.TotalSeconds, 3);
            Log.LogInfo("SQl executed for [{0}] seconds, affected [{1}] rows, when rows passed in was [{2}].", tsSecondsForQuery, rowCount, dataList.Count);
            return rowCount == dataList.Count;
        }
    }
