...
SET START_DATE=01/21/2019
SET END_DATE=09/15/2019
SET NEXT_RUN_DATE_FILE=%BATCH_ROOT%\RXS610\Input\AutoRerunSuperClaim3.dat
SET LOG_DATES_RAN_FILE=%BATCH_ROOT%\RXS610\Archive\AutoRerunSuperClaim3.log
...
:loop_for_date_range
    ...
    @ECHO [%date% %time%] Executing SuperClaim with RunDate [%runDate%].

	%BATCH_ROOT%\PharmacyBusiness\bin\Wegmans.InterfaceEngine.exe -a RX.PharmacyBusiness.ETL.dll -i DownloadSuperClaim -TargetContent SC -RunDate %runDate%
    IF %ERRORLEVEL% NEQ 0 (
        SET PROGRAM_RETURN_CODE=%ERRORLEVEL%
        @ECHO [%date% %time%] Error [%PROGRAM_RETURN_CODE%] running SuperClaim with RunDate [%runDate%].
        goto wrap_it_up
    ) ELSE (
