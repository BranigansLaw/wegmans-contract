...
FROM (
	...
	FROM TREXONE_DW_DATA.FILL_FACT fillfact
				...
				INNER JOIN (SELECT fd_facility_key, rx_number, sold_date_key, cancel_date_key, max(dispensed_item_lot_number) lot_number, max(DISPENSED_ITEM_EXPIRATION_DATE) exp_date 
							...
							where partition_date BETWEEN To_Date('20220810','YYYYMMDD') AND To_Date('20230101','YYYYMMDD')
	...
	WHERE fillfact.partition_date BETWEEN To_Date('20220810','YYYYMMDD') AND To_Date('20230101','YYYYMMDD') 
    and patient.PD_PATIENT_NUM NOT IN (
             39658007
            ,3910778
            ,7714899
            ,14030093
	...
	AND (   --Sold and not cancelled on dtSold date
		(       fillfact.SOLD_DATE_KEY between 20220810 and 20221001
		...
		(       fillfact.cancel_date_key between 20220810 and 20221001
			AND fillfact.SOLD_DATE_KEY between 20220810 and 20221001)
		)
	AND (fac.FD_IS_PPI_ENABLED = 'Y' --Licensed stores
		 OR
		 fac.FD_FACILITY_ID IN ('275','299') --Allow testing in Production through test stores.
		)
	AND drug.DRUG_NDC IN ('33332032203')
