...
{
    public class SnowflakeAlternativePaymentsExport : PharmacyCommandBase
    {
        ...
        public SnowflakeAlternativePaymentsExport(
        ...
        [Command(
            "snowflake-alternative-payments-export",
            Description = "Runs snowflake query, Alternative_Payments_YYYYMMDD.sql",
            Aliases = ["INN615"]
        )]
        public async Task RunAsync(RunForParameter runFor)
        {
            _logger.LogInformation("Gathering AtebMaster data");
            IEnumerable<DeceasedRow> deceasedData = []; // Call Alternative_Payments_YYYYMMDD.sql

            _logger.LogInformation($"Returned {deceasedData.Count()} rows");

            _logger.LogInformation("Write records to file");
            string writePath = _snowflakeDataOutputDirectoriesOptions.Value.INN615.Replace("yyyyMMdd", DateTime.Now.ToString("yyyyMMdd"));
