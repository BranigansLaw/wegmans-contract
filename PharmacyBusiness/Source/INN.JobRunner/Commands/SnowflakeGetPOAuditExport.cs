...
{
    public class SnowflakeGetPOAuditExport : PharmacyCommandBase
    {
        ...
        public SnowflakeGetPOAuditExport(
        ...
        [Command(
            "snowflake-po-audit-export",
            Description = "Runs snowflake query, GetPOAudit_YYYYMMDD.sql",
            Aliases = ["INN613"]
        )]
        public async Task RunAsync(RunForParameter runFor)
        {
            _logger.LogInformation("Gathering AtebMaster data");
            IEnumerable<DeceasedRow> deceasedData = []; // Call GetPOAudit_YYYYMMDD.sql

            _logger.LogInformation($"Returned {deceasedData.Count()} rows");

            _logger.LogInformation("Write records to file");
            string writePath = _snowflakeDataOutputDirectoriesOptions.Value.INN613.Replace("yyyyMMdd", DateTime.Now.ToString("yyyyMMdd"));
