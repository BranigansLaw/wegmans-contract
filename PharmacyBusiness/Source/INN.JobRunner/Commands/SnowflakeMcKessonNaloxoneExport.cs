...
{
    public class SnowflakeMcKessonNaloxoneExport : PharmacyCommandBase
    {
        ...
        public SnowflakeMcKessonNaloxoneExport(
        ...
        [Command(
            "snowflake-mckesson-naloxone-export",
            Description = "Runs snowflake query, McKesson_DW_Naloxone.sql",
            Aliases = ["INN802"]
        )]
        public async Task RunAsync(RunForParameter runFor)
        {
            _logger.LogInformation("Gathering AtebMaster data");
            IEnumerable<DeceasedRow> deceasedData = []; // Call McKesson_DW_Naloxone.sql

            _logger.LogInformation($"Returned {deceasedData.Count()} rows");

            _logger.LogInformation("Write records to file");
            string writePath = _snowflakeDataOutputDirectoriesOptions.Value.INN802.Replace("yyyyMMdd", DateTime.Now.ToString("yyyyMMdd"));
