...
{
    public class SnowflakeGetClaimsExport : PharmacyCommandBase
    {
        ...
        public SnowflakeGetClaimsExport(
        ...
        [Command(
            "snowflake-get-claims-export",
            Description = "Runs snowflake query, GetClaims.sql",
            Aliases = ["INN621"]
        )]
        public async Task RunAsync(RunForParameter runFor)
        {
            _logger.LogInformation("Gathering AtebMaster data");
            IEnumerable<DeceasedRow> deceasedData = []; // Call GetClaims.sql

            _logger.LogInformation($"Returned {deceasedData.Count()} rows");

            _logger.LogInformation("Write records to file");
            string writePath = _snowflakeDataOutputDirectoriesOptions.Value.INN621.Replace("yyyyMMdd", DateTime.Now.ToString("yyyyMMdd"));
