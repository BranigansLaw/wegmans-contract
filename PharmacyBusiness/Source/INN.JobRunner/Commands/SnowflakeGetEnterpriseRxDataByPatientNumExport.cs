...
{
    public class SnowflakeGetEnterpriseRxDataByPatientNumExport : PharmacyCommandBase
    {
        ...
        public SnowflakeGetEnterpriseRxDataByPatientNumExport(
        ...
        [Command(
            "snowflake-enterprise-rx-data-by-patient-num-export",
            Description = "Runs snowflake query, GetEnterpriseRxDataByPatientNum.sql",
            Aliases = ["INN612"]
        )]
        public async Task RunAsync(RunForParameter runFor)
        {
            _logger.LogInformation("Gathering AtebMaster data");
            IEnumerable<DeceasedRow> deceasedData = []; // Call GetEnterpriseRxDataByPatientNum.sql

            _logger.LogInformation($"Returned {deceasedData.Count()} rows");

            _logger.LogInformation("Write records to file");
            string writePath = _snowflakeDataOutputDirectoriesOptions.Value.INN612.Replace("yyyyMMdd", DateTime.Now.ToString("yyyyMMdd"));
