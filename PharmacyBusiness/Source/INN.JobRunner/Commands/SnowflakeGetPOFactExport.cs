...
{
    public class SnowflakeGetPOFactExport : PharmacyCommandBase
    {
        ...
        public SnowflakeGetPOFactExport(
        ...
        [Command(
            "snowflake-po-fact-export",
            Description = "Runs snowflake query, GetPOFact_YYYYMMDD.sql",
            Aliases = ["INN614"]
        )]
        public async Task RunAsync(RunForParameter runFor)
        {
            _logger.LogInformation("Gathering AtebMaster data");
            IEnumerable<DeceasedRow> deceasedData = []; // Call GetPOFact_YYYYMMDD.sql

            _logger.LogInformation($"Returned {deceasedData.Count()} rows");

            _logger.LogInformation("Write records to file");
            string writePath = _snowflakeDataOutputDirectoriesOptions.Value.INN614.Replace("yyyyMMdd", DateTime.Now.ToString("yyyyMMdd"));
