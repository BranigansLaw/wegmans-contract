...
{
    public class SnowflakeErxUnauthorizedExport : PharmacyCommandBase
    {
        ...
        public SnowflakeErxUnauthorizedExport(
        ...
        [Command(
            "snowflake-erx-unauthorized-export",
            Description = "Runs snowflake query, SelectERxUnauthorized.sql",
            Aliases = ["INN703"]
        )]
        public async Task RunAsync(RunForParameter runFor)
        {
            _logger.LogInformation("Gathering AtebMaster data");
            IEnumerable<DeceasedRow> deceasedData = []; // Call SelectERxUnauthorized.sql

            _logger.LogInformation($"Returned {deceasedData.Count()} rows");

            _logger.LogInformation("Write records to file");
            string writePath = _snowflakeDataOutputDirectoriesOptions.Value.INN703.Replace("yyyyMMdd", DateTime.Now.ToString("yyyyMMdd"));
