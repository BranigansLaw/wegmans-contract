...
{
    public class SnowflakeSuperWorkflowExport : PharmacyCommandBase
    {
        ...
        public SnowflakeSuperWorkflowExport(
        ...
        [Command(
            "snowflake-super-workflow-export",
            Description = "Runs snowflake query, Super_Workflow_Steps_YYYYMMDD.sql",
            Aliases = ["INN549"]
        )]
        public async Task RunAsync(RunForParameter runFor)
        {
            _logger.LogInformation("Gathering WorkersComp data");
            IEnumerable<DeceasedRow> deceasedData = []; // Call Super_Workflow_Steps_YYYYMMDD.sql

            _logger.LogInformation($"Returned {deceasedData.Count()} rows");

            _logger.LogInformation("Write records to file");
            string writePath = _snowflakeDataOutputDirectoriesOptions.Value.INN549.Replace("yyyyMMdd", DateTime.Now.ToString("yyyyMMdd"));
