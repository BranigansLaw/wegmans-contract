...
{
    public class SnowflakeAtebMasterExport : PharmacyCommandBase
    {
        ...
        public SnowflakeAtebMasterExport(
        ...
        [Command(
            "snowflake-ateb-master-export",
            Description = "Runs snowflake query, AtebMaster_YYYYMMDD.sql",
            Aliases = ["INN550"]
        )]
        public async Task RunAsync(RunForParameter runFor)
        {
            _logger.LogInformation("Gathering AtebMaster data");
            IEnumerable<DeceasedRow> deceasedData = []; // Call AtebMaster_YYYYMMDD.sql

            _logger.LogInformation($"Returned {deceasedData.Count()} rows");

            _logger.LogInformation("Write records to file");
            string writePath = _snowflakeDataOutputDirectoriesOptions.Value.INN550.Replace("yyyyMMdd", DateTime.Now.ToString("yyyyMMdd"));
