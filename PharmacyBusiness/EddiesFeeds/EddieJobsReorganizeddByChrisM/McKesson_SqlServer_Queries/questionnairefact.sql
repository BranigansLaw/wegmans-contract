...
FROM [cps-ERX-WEGP01-ERX].[CPS_QUESTIONNAIRE_FACT] quesfact
...
LEFT OUTER JOIN [cps-ERX-WEGP01-ERX].CPS_MESSAGE_FACT msgfact
ON quesfact.CPS_MESSAGE_NUM = msgfact.CPS_MESSAGE_NUM

WHERE convert(date,QUESFACT.RESOLVED_DATE) = convert(date, (GETDATE()-1))

--WHERE quesfact.RESOLVED_DATE BETWEEN
