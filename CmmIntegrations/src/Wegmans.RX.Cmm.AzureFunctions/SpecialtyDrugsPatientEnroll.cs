...
{
    public class SpecialtyDrugsPatientEnroll
    {
        private AuthorizationOptions _adSettings;
        private AstuteSoapProxy _astuteSoapProxy;

        public SpecialtyDrugsPatientEnroll(IOptions<AuthorizationOptions> adSettings, IAstuteSoapProxy astuteSoapProxy)
        {
        ...
        [FunctionName(nameof(SpecialtyDrugsPatientEnroll))]
        public async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "specialtydrugs/patients/enroll")] HttpRequest request,
            ILogger log
            )
        {
            ...
            if (!(principal is null) && principal.IsInRole("Cmm.Writer"))
            {
                ...
                Cmm.Models.Enrollment enrollment = JsonSerializer.Deserialize<Cmm.Models.Enrollment>(await new StreamReader(request.Body).ReadToEndAsync().ConfigureAwait(false), options);
                ...
                log.LogDebug("Before Translating CMM Enrollment object into Astute Enrollment object.");

                var astuteEnrollment = enrollment.CreateAstuteEnrollment();

                log.LogDebug("After Translating CMM Enrollment object into Astute Enrollment object.");
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        await _astuteSoapProxy.UpdatePatientAsync(patientCase, astuteEnrollment.Patient).ConfigureAwait(false);
                    }
